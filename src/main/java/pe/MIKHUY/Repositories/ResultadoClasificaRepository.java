...
public interface ResultadoClasificaRepository extends JpaRepository<ResultadoClasifica, UUID> {
    ...
    @Query("SELECT r FROM ResultadoClasifica r WHERE r.sesion.progreso.estudiante.id = :estudianteId ORDER BY r.nivelNumero ASC")
    List<ResultadoClasifica> findBySesionEstudianteId(@Param("estudianteId") UUID estudianteId);
}